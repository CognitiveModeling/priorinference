...
#x1pilotData <- read.csv("X1_data/x1pilot_sRSA_DataCrossValAugmentedfixed_2019_0429.csv")
#x1pilotData <- read.csv("X1_data/x1pilot_sRSA_DataCrossValAugmented_2019_0507.csv")
x1pilotData <- read.csv("X1_data/x1pilot_sRSA_DataCrossValAugmented2_2019_0507.csv")

#x1pilotData <- read.csv("X1_data/x1pilot_DataCrossValAugmentedfixed_2019_0429.csv")
#x1pilotData <- read.csv("X1_data/x1pilot_DataCrossValAugmented_2019_0429.csv")
#x1pilotData <- read.csv("X1_data/x1pilot_DataCrossValAugmented_notObey.1_2019_0429.csv")
#x1pilotData <- read.csv("X1_data/x1pilot_DataCrossValAugmented23_2019_0429.csv")
...
### reordering worker and model answers for all trials. 
for(i in c(1:length(x1pilotData$X))) {
  ...
  for(j in c(1:3)) {
    ...
    featValIndex <- 2
    # now determine second and third feature value for the specific object
    for(k in c(2:3)) {
    ...
    if(featValIndex < 4) { # not all feature values assigned, yet (remaining cases have equal property) -> fill up
...
for(i in c(1:length(myCCodes))) {
  ...
  ccSpecificTrials <- x1pilotData[cases,]
  for(j in c(1:9)) {
    specCases <- which(is.na(ccSpecificTrials[,subjectGuessIndex-1+j] ) == FALSE)
    if(length(specCases) > 0) {
#      if(mean(ccSpecificTrials[specCases,(modelGuessIndex4-1+j)]) < 1/3 - 1e-5
#         | mean(ccSpecificTrials[specCases,(modelGuessIndex4-1+j)]) > 1/3 + 1e-5) {
        dataPointIndex <- dataPointIndex + 1
        workerData[dataPointIndex] <- mean(ccSpecificTrials[specCases,(subjectGuessIndex-1+j)])
        rsaModel[dataPointIndex] <- mean(ccSpecificTrials[specCases,(modelGuessIndex1-1+j)])
        rsaModel2[dataPointIndex] <- mean(ccSpecificTrials[specCases,(modelGuessIndex4-1+j)])
#      }
    }
  }
}
