...
startPos <- .5 # (in percentage of data length, 0 = from the start)
endPos <- 1   # (in percentage of data length, 1 = end of data)
...
for(workerID in c(0:idMax)) {
  ...
  if(length(idICases)>0) { # data valid
    # selecting a subset of the individual data.
    idICases <- idICases[round(1 +  startPos * (length(idICases)-1)) : round( 1 + endPos * (length(idICases)-1))]
...
klDivsOutFileName <- paste("X2_data/x2KLDivs_sRSA_indOpt_S",startPos,"_E",endPos,"_2021_03_01.csv",sep="")
paramsOutFileName <- paste("X2_data/x2Params_sRSA_indOpt_S",startPos,"_E",endPos,"_2021_03_01.csv",sep="")
write.csv(klDivUttWorkers, klDivsOutFileName)
write.csv(paramsUttWorkers, paramsOutFileName)

# Optimization Routines and Results from 2019:
